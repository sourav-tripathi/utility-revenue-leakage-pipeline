...
Executive Summary
The business was experiencing unexplained revenue leakage. The objective was to ingest, clean, and analyze 4.5 million legacy billing records to identify the root cause. Through Exploratory Data Analysis (EDA) and advanced Window Functions, I proved the leakage was not a system failure, but localized hardware tampering (meter bypassing). I isolated 27,051 high-probability fraudulent accounts and engineered a dedicated Data Mart to feed a live Executive Power BI Dashboard.

Phase 1: Ingestion & Security Architecture
...
Interview Talking Point (Security): > “I enforced the Principle of Least Privilege (PoLP). Bulk data insertion was handled by a dedicated pipeline_admin account, ensuring that end-user reporting tools do not have global root access to delete or alter financial records.”
...
Phase 3: ELT Data Transformation
...
The Action: Bypassed the database safety lock (SET SQL_SAFE_UPDATES = 0), created a strict DATE column, and executed a dynamic CASE WHEN statement to standardize the entire 4.5-million-row ledger.
...
The Logic Behind the Filter (WHERE current_usage = 0 AND previous_usage > 500):

Why current_usage = 0? We are hunting for meters that have stopped recording.

Why previous_usage > 500? If a customer drops from 20 units to 0, they likely just went on a long vacation. However, 500+ units indicates a fully active, high-consumption household. A sudden, violent drop from massive consumption to absolute zero is physically unnatural. It is the exact digital fingerprint of a customer physically bypassing the meter to steal electricity, or a catastrophic meter hardware failure.
